/**
 * services.js - 动态加载 Services 信息并渲染到页面（中文版）
 * 
 * 工作方式：
 * 1. 首先尝试通过 <script> 加载 services-cn/services-data.js 数据脚本
 * 2. 如果加载成功，使用其中的数据渲染
...
(function() {
    ...
    function renderServices(services) {
        ...
        if (!services || services.length === 0) {
            serviceGrid.innerHTML = '<p style="text-align: center; color: #999; grid-column: 1 / -1;">暂无可用服务。</p>';
            return;
        }

        services.forEach(function(service) {
            var card = document.createElement('a');
            card.href = 'services-cn/' + service.detailPage;
    ...
    if (window.__SERVICES_DATA__ && window.__SERVICES_DATA__.services) {
        ...
        script.src = 'services-cn/services-data.js';
